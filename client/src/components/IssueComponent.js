// Refactored IssueComponent with simplified layout
...
const IssueComponent = () => {
	...
	const { isLoading, errMess } = useSelector(state => state.issues);
	...
	return (
		<div className="container mt-4">
			<h2 className="heading text-center">Issue Books</h2>
			<hr />

			<div className="row">
				{books.map(book => (
					<div className="col-md-4 col-12 mb-3" key={book._id}>
						<div className="border rounded p-3">
							<p><strong>{book.name}</strong> by {book.author}</p>
							<p><strong>Available Copies:</strong> {book.copies}</p>
							<button className="btn btn-success btn-sm" onClick={() => handleIssue(book._id)}>
								Issue Book
							</button>
						</div>
					</div>
