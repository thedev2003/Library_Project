// Refactored HistoryComponent with cleaner layout
...
const HistoryComponent = () => {
  const { issues, isLoading, errMess } = useSelector(state => state.issues);
  ...
  return (
    <div className="container mt-4">
      <h2 className="heading text-center">Issue History</h2>
      <hr />

      <div className="row">
        {issues.map(issue => (
          <div className="col-md-6 col-12 mb-3" key={issue._id}>
            <div className="border rounded p-3">
              <p><strong>Book:</strong> {issue.bookname}</p>
              <p><strong>Issued On:</strong> {new Date(issue.issuedate).toLocaleDateString()}</p>
              <p><strong>Returned:</strong> {issue.status === 'returned' ? 'Yes' : 'No'}</p>
            </div>
          </div>
        ))}
      </div>
