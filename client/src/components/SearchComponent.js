// Refactored SearchComponent with conditional results and clean logic
...
const SearchComponent = ({ books, booksLoading, booksErrMess, isSignedIn, isAdmin, toggleEditModal, toggleDeleteModal, changeSelected }) => {
      ...
      <div className="row">
        {filteredBooks.map(book => (
          <div key={book._id} className="col-md-3 col-6 mb-4">
            <Card>
              <CardImg top src={book.image} alt={book.name} height="280" />
              <CardBody>
                <CardTitle tag="h5">{book.name}</CardTitle>
                <CardText><small className="text-muted">{book.author}</small></CardText>
                {isAdmin && (
                  <div className="mt-2 text-center">
                    <Button size="sm" color="info" className="me-2" onClick={() => { toggleEditModal(); changeSelected(book._id); }}>Edit</Button>
                    <Button size="sm" color="danger" onClick={() => { toggleDeleteModal(); changeSelected(book._id); }}>Delete</Button>
                  </div>
                )}
              </CardBody>
            </Card>
